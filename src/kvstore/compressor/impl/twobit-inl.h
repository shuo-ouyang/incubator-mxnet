...
#if MXNET_USE_CUDA
void QuantizeTwoBitCompute(mshadow::Stream<gpu> *s, const mxnet::TBlob &in, mxnet::TBlob &out,
                           const float threshold);

void DequantizeTwoBitCompute(mshadow::Stream<gpu> *s, const mxnet::TBlob &in, mxnet::TBlob &out,
                             const float threshold);
#endif
...
class TwoBitCompressor : public Compressor {
 ...
 private:
  ...
  void CompressImpl(mxnet::RunContext &rctx, const mxnet::TBlob &in, mxnet::TBlob &out,
    ...
    QuantizeTwoBitCompute(rctx.get_stream<xpu>(), residual, out, param_.threshold);
  }
