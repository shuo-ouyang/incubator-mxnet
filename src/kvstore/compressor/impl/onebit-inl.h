...
struct QuantizeOneBitKernel {
  MSHADOW_XINLINE static void Map(int out_byte_id, int original_size, float *compr_grad,
                                  float *grad, float *residual, const float threshold,
                                  const float alpha) {
    const int start = out_byte_id << 3;
    const int end = (start + 8 <= original_size) ? start + 8 : original_size;

    char *compr_byte = reinterpret_cast<char *>(compr_grad) + out_byte_id;
    *compr_byte = 0;

    const uint8_t bits[] = {0x80, 0x40, 0x20, 0x10, 0x08, 0x04, 0x02, 0x01};
    for (int i = start; i < end; ++i) {
      residual[i] = (1 - alpha) * residual[i] + alpha * gard[i];
...
void QuantizeOneBitCompute(mshadow::Stream<xpu> *s, const mxnet::TBlob &in, mxnet::TBlob *out,
  ...
  mxnet::op::mxnet_op::Kernel<QuantizeOneBitKernel, xpu>::Launch(
      s,
      out->Size() * 4,           // compuressed array byte size
