...
bool GradientCompression::SupportFastAggregate() const { return compr_->SupportFastAggregate(); }
...
void GradientCompression::DecompressAndAggregateEx(const mxnet::NDArray &from, mxnet::NDArray *to,
                                                   const int priority) {
  CHECK(compr_->SupportFastAggregate())
      << "current compressor does not support fast dequantize and aggregate";
  CHECK(shape_is_known(from.shape())) << "source operand has undefined shape";
  CHECK(shape_is_known(to->shape())) << "destination operand has undefined shape";
  const int a = from.ctx().dev_mask();
  const int b = to->ctx().dev_mask();
  if (a == mshadow::cpu::kDevMask and b == mshadow::cpu::kDevMask) {
    mxnet::Engine::Get()->PushSync(
        [this, from, to](mxnet::RunContext rctx) {
          compr_->DecompressAndAggregate(rctx, from.data(),
                                         const_cast<mxnet::TBlob *>(&to->data()));
        },
        from.ctx(), {from.var()}, {to->var()}, mxnet::FnProperty::kNormal, priority,
        "DecompressAndAggregateCPU");
  } else {
#if MXNET_USE_CUDA
    if (a == mshadow::gpu::kDevMask and b == mshadow::gpu::kDevMask) {
      mxnet::Engine::Get()->PushSync(
