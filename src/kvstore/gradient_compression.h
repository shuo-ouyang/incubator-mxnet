...
class GradientCompression {
 public:
  ...
  /*!
   * \brief returns whether compressor support fast aggregate method without dequantization
   */
  bool SupportFastAggregate() const;
  ...
  void DecompressEx(const mxnet::NDArray &from, mxnet::NDArray *to, const int priority);
    ...
    /*!
   * \brief Issues dequantize and aggregate operation to be scheduled by the engine
   * Decompresses `from` into `to` using current parameters of `type` and `threshold`
   * \param from the ndarray containing quantized data
   * \param to the target ndarray which contains final dequantized data
   * \param priority Priority of the action.
   */
  void DecompressAndAggregateEx(const mxnet::NDArray &from, mxnet::NDArray *to, const int priority);
